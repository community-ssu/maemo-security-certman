...
class c_xmlnode 
{
 public:
  /// \brief Constructor
  /// \param of_doc The c_xmldoc-instance where the node belongs to
  /// \param of_parent The node's parent (NULL, if this is root node)
  /// \param of_name The name of the element to create

  c_xmlnode (c_xmldoc* of_doc, c_xmlnode* of_parent, const char* of_name);

  /// \brief Destructor
  ~c_xmlnode ();

  /// \brief Return a pointer to the enclosing document
  /// \return An c_xmldoc-pointer that was given in the
  /// constructor
  ...
  /// \brief Return the contents of a subtree as a c_fstring
  ...
  c_xmlnode* child (char* of_name, bool required);
 ...
 private:
  ...
  c_fstring content_str;
  c_map attributes;
  // c_farray<c_fstring> attr_names;
  // c_farray<c_fstring> attr_values;
  c_farray<c_xmlnode> children;
